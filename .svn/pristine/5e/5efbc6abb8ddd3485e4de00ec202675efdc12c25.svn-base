var UserDTU = {
    ...
    SaerchSiteUser: function (srcObj, dtuId) {
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    str += "<tr>";
                    str += "<td class='com_w20pen com_TextCenter' >" + data[i].UserName + "</td>";
                    str += "<td class='com_w20pen com_TextCenter'>" + data[i].EmpName + "</td>";
                    str += "<td class='com_w20pen com_TextCenter'>" + data[i].IDSName + "</td>";
                    str += "<td class='com_w20pen com_TextCenter'>" + data[i].EmpTelphone + "</td>";
    ...
    //添加用户
    AddUser: function (dtuId) {
        if (!dtuId) {
            layer.alert("请先选择站点！", 8);
            return;
        }
        Common.WinTopBox("用户添加", "60%", "83%", "10%", "20%", "UserInfo/AddUser?dtuId=" + dtuId);
    },
    ...
    //根据公司查询用户列表
    GetUserListByCompany: function (companyId, siteId) {
        $.ajax({
            type: "post",
            url: "/UserInfo/GetUserListByCompanyId",
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    str += "<tr>";
                    str += "<td class='com_w30pen com_TextCenter'><input type='checkbox' value='" + data[i].UserId + "' name='" + siteId + "' /></td>";
                    str += "<td class='com_w35pen com_TextCenter'>" + data[i].UserId + "</td>";
                    str += "<td class='com_w35pen com_TextCenter'>" + data[i].TrueName + "</td>";
                    str += "</tr>";
                }
                $("#tbUsers tbody").html(str);
            }
        });
    }
}