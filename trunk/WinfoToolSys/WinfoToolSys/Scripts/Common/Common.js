var Common = {
    ...
    DtuConfigList: function () {
        $.ajax({
            ...
            success: function (data) {
                var str = "";
                for (var i = 0; i < data.length; i++) {
                    str += '<option value="' + data[i].ConfigCode + '">' + data[i].ConfigName + '</option>';
                }
                $("#DtuConfig").html(str);
            }
        });
    ...
    layoutClose: function () {
        $('.msbgs .myBG .myBGTop ', top.document).fadeTo("slow", 0);
        $("#ifrHTML", top.document).attr("src", "");
        setTimeout(function () { $('.msbgs', top.document).hide(); }, "500");
    },
    //关闭弹出顶层
    layoutTopClose: function () {
        $('.myBGTop', top.document).fadeTo("slow", 0).css("z-index", -1);
        $('#WinTop', top.document).hide().css("z-index",  1);
        $("#ifrHTMLTop", top.document).attr("src", "");
    },
